...
{
	return (CurrentActorInfo ? Cast<UGHAbilitySystemComponent>(CurrentActorInfo->AbilitySystemComponent.Get()): nullptr);
}
...
void UGHGameplayAbility::OnGiveAbility(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& AbilitySpec)
{
	Super::OnGiveAbility(ActorInfo, AbilitySpec);
}
...
void UGHGameplayAbility::EndCurrentAbility(bool is_replicate_end_ability, bool is_was_cancelled)
{
	if (IsActive())
	{
		EndAbility(GetCurrentAbilitySpecHandle(), GetCurrentActorInfo(), GetCurrentActivationInfo(), is_replicate_end_ability, is_was_cancelled);
	}
}
